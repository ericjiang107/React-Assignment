...
export const DroneForm = (props: DroneFormProps) => {
    ...
    const onSubmit = (data:any, event:any) => {
        ...
        if(props.id!) { // (!) means if it is not null or if it is there
            server_calls.update(props.id, data)
            console.log(`Updated: ${data} ${props.id}`) // this is for trouble shooting
            window.location.reload()
            event.target.reset();
            window.location.reload()
        } else {
            dispatch(chooseName(data.name))
            // after dispatch, change current state:
